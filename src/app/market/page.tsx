...
export default async function MarketPage({ searchParams }: Props) {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl py-6 sm:py-8">
      <div className="mb-4 flex flex-col gap-3 sm:mb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight text-zinc-900 sm:text-2xl">장터</h1>
          <p className="mt-1 text-sm text-zinc-600">시세가 붙은 원문 매물입니다. 거래는 당근·번개에서 하세요.</p>
        </div>
        <Link href="/market/new" className="bg-zinc-900 px-4 py-2 text-center text-sm font-medium text-white sm:shrink-0">
          원문 올리기
        </Link>
      </div>
      ...
      <form className="mb-5 flex flex-col gap-2 sm:flex-row">
        <input
          ...
          className="w-full border border-zinc-300 bg-white px-3 py-2.5 text-base outline-none focus:border-zinc-500 sm:flex-1 sm:text-sm"
        />
        <button type="submit" className="border border-zinc-300 bg-white px-4 py-2.5 text-sm text-zinc-700 sm:w-auto">
          검색
        </button>
      </form>
      ...
      {listings.length === 0 ? (
        <div className="border border-zinc-200 bg-white px-5 py-12 text-center">
          ...
          <Link href="/market/new" className="mt-4 inline-flex bg-zinc-900 px-4 py-2 text-sm text-white">
            첫 매물 올리기
          </Link>
        </div>
      ) : (
          ...
          {listings.map((item) => {
            const body = (
              <>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {item.isFairVerified ? (
                    <span className="shrink-0 border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700">
                      적정
                    </span>
                  ) : item.verdict && VERDICT_KO[item.verdict] ? (
                    <span className={`shrink-0 border px-2 py-0.5 text-[11px] font-medium ${VERDICT_STYLE[item.verdict] ?? ""}`}>
            ...
            return item.sourceUrl ? (
              <a
                ...
                className="border border-zinc-200 bg-white p-4 hover:border-zinc-300"
              >
                {body}
              </a>
            ) : (
