...
function buildWeeklyAverages(trend: PriceProfileData["trend"] | null | undefined) {
  ...
  if (!part || !summary) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16 text-center">
        <p className="text-sm text-zinc-500">부품 정보를 불러올 수 없습니다.</p>
      </main>
    );
  }

  const weeklyTrend = buildWeeklyAverages(trend);
  const hasTrendChart = trend.length > 0 && weeklyTrend.length > 0;
  const trendMax = hasTrendChart ? Math.max(...weeklyTrend.map((w) => w.avgPrice)) : 0;
  const trendMin = hasTrendChart ? Math.min(...weeklyTrend.map((w) => w.avgPrice)) : 0;

  const hasNewPrice =
    summary.newPrice != null && Number.isFinite(summary.newPrice) && summary.newPrice > 0;
  const hasDepreciation =
    hasNewPrice &&
    summary.depreciationPct != null &&
    Number.isFinite(summary.depreciationPct);
    ...
    <main className="mx-auto max-w-2xl space-y-6 px-4 py-8">
      ...
      <header>
        ...
        <p className="mt-2 text-sm text-zinc-500">
          실거래 {sampleSize.toLocaleString("ko-KR")}건 기준 · 최근 60일
        </p>
      </header>
      ...
      <section className="rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-700">
        {hasNewPrice ? (
          ...
          <span className="text-zinc-500">신품가 정보 없음</span>
        )}
      </section>
